...
import { detailFields } from '@/lib/dataEquipment';
import { MachineItem, MapItem } from '@/lib/typeMachine';
    ...
    <div className={`px-2 sm:px-10 pb-2`}>
                    ...
                    <li key={field} className="text-slate-400 px-2">
                      ...
                      >
                        ...
                        /F$/.test(field) ? (
                          ...
                          <div
                            className="font-medium text-xl bg-yellow-200 text-blue-500 hover:text-blue-600 transition-colors duration-300 ease-in-out cursor-pointer flex items-center p-4 border border-gray-300 rounded-lg shadow-lg hover:shadow-xl"
                            onClick={() =>
                              handleShowForm(item['id'], item['collection'])
                            }
                          >
                            <div className="flex-shrink-0">
                              <img
                                src={`/assets/icons/${item[
                                  'collection'
                                ]?.toLowerCase()}.svg`}
                                alt={item.id}
                            ...
                            <span className="ml-4 text-blue-700">
                              {item[field as keyof MachineItem]} <br />
      ...
      {formVisible && selectedForm.id !== undefined && (
        <ModalForm
          bu={'vn'}
          id={selectedForm.id}
          machine={selectedForm.collection}
