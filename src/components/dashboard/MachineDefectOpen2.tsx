...
const Detail = ({ dataTr, item }: MachineProps) => {
  ...
  const filterOpenIssue = (data: Machine[]): FilteredMachineItem[] => {
    return data
  ...
  const openIssue = filterOpenIssue(dataTr);
  ...
  return (
    <div className={`pb-2`}>
      <div className="flex justify-center items-center mb-2 text-3xl font-bold text-slate-900">
        <span className="text-slate-300 mx-1">Open </span>
        <span className="text-bold text-rose-500">{openIssue.length}</span>
