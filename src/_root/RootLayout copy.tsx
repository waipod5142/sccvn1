...
const RootLayout = () => {
  ...
  useEffect(() => {
    if (localStorageData.length > 0) {
      const redirectTo = `/Man/vn/Toolbox/${localStorageData[0].id}`;
  ...
  const onSubmit = async (formData: FieldValues) => {
    ...
    reset();
    window.location.href = `/Man/Toolbox/${formData.id
      .replace(/[/\s]/g, '-')
      .toUpperCase()}`;
  };
      ...
      <div className="my-4">
        ...
        {localStorageData.map((data, index) => (
          <div
            ...
            onClick={() =>
              (window.location.href = `/Man/Toolbox/${localStorageData.map(
                (data) => data.id
              )}`)
            }
          >
